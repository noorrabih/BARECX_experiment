...
def submit_fold(client, fold: str, out_dir: str) -> str:
    """Upload JSONL and create a batch job for one fold. Returns job name."""
    jsonl_path = f"{out_dir}/batch_fold_{fold}.jsonl"
...
def save_results(rows_a: list[dict], rows_b: list[dict], out_dir: str):
    ...
    df_folds = pd.read_csv(f"{out_dir}/fold_assignments.csv")[["ID", "fold"]]
    ...
    out_path = f"{out_dir}/gemini_results_X4.csv"
...
def main():
    ...
    parser.add_argument("--n-shots", type=int, default=1, choices=range(1, 5),
                        metavar="{1-4}", help="Number of shots used when preparing the batch (determines input directory)")
    ...
    out_dir = f"X4/{args.n_shots}_shots"
    ...
    if args.submit:
        fold_a_job_name = submit_fold(client, "a", out_dir)
        fold_b_job_name = submit_fold(client, "b", out_dir)
        ...
        if args.wait:
            ...
            save_results(rows_a, rows_b, out_dir)
